...
void Enemy::update(float& elapsedTime, Vector2f playerPos) {
   ...
   switch (enemyType) {
   case 0:
      pVelX = (ENEMY1_SPEED / distance) * (distanceX)+pAccX;
      pVelY = (ENEMY1_SPEED / distance) * (distanceY)+pAccY;
      pPosition.x += pVelX;
      pPosition.y += pVelY;
      break;
   case 1:
   ...
   case 2:
      if (distance < 400) {
         pAccX = (ENEMY1_ACCEL / distance) * (distanceX);
         pAccY = (ENEMY1_ACCEL / distance) * (distanceY);
         pVelX += pAccX;
         pVelY += pAccY;
      }
      else {
